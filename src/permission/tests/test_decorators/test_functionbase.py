...
from permission.tests.test_decorators.utils import \
        create_mock_permission_handler
...
class PermissionFunctionDecoratorsTestCase(TestCase):
    def setUp(self):
        self.permission_handler = create_mock_permission_handler()
        self.request = create_mock_request(self.permission_handler)
    ...
    def test_list_detail_object_id(self):
        ...
        self.permission_handler.has_perm.return_value = False
        ...
        self.permission_handler.has_perm.assert_called_with(
                'permission.add_article',
                obj=self.model_instance,
            )
        ...
        self.permission_handler.has_perm.return_value = True
        self.decorated(
                self.request, 
                queryset=self.queryset,
                object_id=1)
